class Offer < ActiveRecord::Base
  belongs_to :merchant, foreign_key: :merchant_id, primary_key: :merchant_id
  ...
  def self.parse_xml response
    Nokogiri::XML(response.body).xpath("/cj-api/links/link").each do |link|
      merchant = Merchant.where(merchant_id: link.xpath("advertiser-id").text.to_i, \
      ...
      Offer.where(offer_id: link.xpath("link-id").text.to_i, \
                  ...
                  expires_at: link.xpath("promotion-end-date").text.to_time 
                  ).first_or_create
    end
  end
  def self.crawl
